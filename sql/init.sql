...
CREATE TABLE IF NOT EXISTS `user` (
    ...
    `exam_year` INT DEFAULT NULL COMMENT '考研年份',
...
CREATE TABLE IF NOT EXISTS `question` (
    ...
    `year` INT DEFAULT NULL COMMENT '年份（真题用）',
    `sort` INT DEFAULT 0 COMMENT '排序',
    `status` TINYINT DEFAULT 1 COMMENT '状态 0禁用 1启用',
    `create_time` DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `update_time` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
...
CREATE TABLE IF NOT EXISTS `course_video` (
    ...
    `video_url` VARCHAR(500) DEFAULT NULL COMMENT '视频地址（腾讯云COS）',
    `video_key` VARCHAR(255) DEFAULT NULL COMMENT '视频key（用于生成签名URL）',
    `duration` INT DEFAULT 0 COMMENT '时长（秒）',
    `is_free` TINYINT DEFAULT 0 COMMENT '是否试看 0否 1是',
    `sort` INT DEFAULT 0 COMMENT '排序',
    `status` TINYINT DEFAULT 1 COMMENT '状态 0禁用 1启用',
    `create_time` DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `update_time` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
...
CREATE TABLE IF NOT EXISTS `user_course_progress` (
    ...
    `course_id` BIGINT NOT NULL COMMENT '课程ID',
    `video_id` BIGINT NOT NULL COMMENT '当前观看视频ID',
    `progress` INT DEFAULT 0 COMMENT '播放进度（秒）',
    `is_completed` TINYINT DEFAULT 0 COMMENT '是否看完 0否 1是',
    `create_time` DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `update_time` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `practice_record` (
    ...
    `user_id` BIGINT NOT NULL COMMENT '用户ID',
    `major_id` BIGINT NOT NULL COMMENT '专业ID',
    `category_id` BIGINT DEFAULT NULL COMMENT '分类ID（可空=随机刷题）',
...
CREATE TABLE IF NOT EXISTS `user_wrong_question` (
    ...
    `correct_count` INT DEFAULT 0 COMMENT '正确次数（用于自动移出）',
